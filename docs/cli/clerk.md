...
3. **Auth gate.** `--apply` refuses to run if `gh auth status` exits non-zero. T7 supports PAT-mode only via the existing `gh` CLI; T6 will introduce a GitHub-App-aware auth abstraction.
