...
**Input:** `/synapse-gatekeeper src/skill/skill-creator --score 84`
...
- [x] Domain README (src/skill/README.md) has row for skill-creator
...
**Input:** `/synapse-gatekeeper src/skill/write-skill-eval --score 76`
...
- [x] Domain README (src/skill/README.md) has row for write-skill-eval
- [x] Name globally unique — no collision in SKILLS_REGISTRY.yaml
...
1. **Eval score below threshold (76/80):** Run `/improve-skill src/skill/write-skill-eval` to identify and fix quality gaps. Re-run `/synapse-gatekeeper` with the updated score once it reaches ≥ 80.
```
...
**Input:** `/synapse-gatekeeper src/skill/write-scope-docs`
...
1. **EVAL.md missing:** No certification is possible without an EVAL.md. Run `/write-skill-eval src/skill/write-scope-docs` to generate one, then re-run `/synapse-gatekeeper` with a measured eval score.
```
