...
Loaded by synapse-gatekeeper when the artifact path points to `src/agents/*.md`. Agents have two tiers — no registry tier (agents are not in SKILLS_REGISTRY.yaml).
...
|-------|---------------|
| Agent file exists | `.md` file is present in `src/agents/` and non-empty |
