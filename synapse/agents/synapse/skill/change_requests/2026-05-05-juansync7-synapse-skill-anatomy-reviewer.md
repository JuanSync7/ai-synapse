...
| `synapse-router-artifact-creator` (flow-skill `[R]` phase) | produces for | Anatomy verdict is one input to the orchestrator's aggregated gate before `[EVAL]` |
| `/synapse-skill-skill-improver` | produces for | Used as structural pre-check; anatomy failures reported before design scoring begins |
...
- **Dispatcher chain:** synapse-router-artifact-creator flow-skill `[draft]` → `[R] synapse-skill-signal-reviewer` (which dispatches this sub-agent) → `[EVAL] write-skill-eval`. Also reachable via `/synapse-skill-skill-improver` structural pre-check.
- **Model:** sonnet.
