...
**Directory placement:** `synapse/skills/synapse-creator/` — path subdomain must equal frontmatter `domain`. Earlier consideration of `synapse/skills/meta/` dropped because the path subdomain must match `domain: synapse` per repo convention.
