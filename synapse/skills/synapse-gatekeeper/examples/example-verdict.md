...
**Input:** `/synapse-gatekeeper synapse/skills/synapse-creator --score 84`
...
- [x] Domain README (synapse/skills/README.md) has row for skill-creator
...
**Input:** `/synapse-gatekeeper synapse/skills/write-synapse-eval --score 76`
...
- [x] Domain README (synapse/skills/README.md) has row for write-skill-eval
- [x] Name globally unique — no collision in registry/SKILL_REGISTRY.md
...
1. **Eval score below threshold (76/80):** Run `/improve-skill synapse/skills/write-synapse-eval` to identify and fix quality gaps. Re-run `/synapse-gatekeeper` with the updated score once it reaches ≥ 80.
```
...
**Input:** `/synapse-gatekeeper src/skills/docs/write-scope-docs`
...
1. **EVAL.md missing:** No certification is possible without an EVAL.md. Run `/write-synapse-eval skill src/skills/docs/write-scope-docs` to generate one, then re-run `/synapse-gatekeeper` with a measured eval score.
```
