# Decision Memo — synapse-router-artifact-creator

> Artifact type: skill | Memo type: creation | Design doc: `../../../../.brainstorms/2026-05-03-synapse-router-artifact-creator-consolidation/design.md`

---

## What I want

A single `synapse-router-artifact-creator` skill that replaces three existing fragmented creators (`skill-creator`, `protocol-creator`, `agent-creator`) and adds a missing fourth (`tool-creator`). The user invokes it once per artifact creation. The skill detects artifact type first, then loads only the matching type-specific flow — no other flow ever loads in the same session. Token cost stays low because only one flow and its references are active at a time.
...
Not applicable — synapse-router-artifact-creator does not use a notepad. It is a single-session, single-artifact workflow.

---

## Naming conventions

**Artifact name:** `synapse-router-artifact-creator` (confirmed in brainstorm turn 5).

**Directory placement:** `synapse/skills/synapse-router-artifact-creator/` — path subdomain must equal frontmatter `domain`. Earlier consideration of `synapse/skills/meta/` dropped because the path subdomain must match `domain: synapse` per repo convention.
...
├── EVAL.md                                 # Gatekeeper EVAL for synapse-router-artifact-creator itself
├── references/
...
| User invokes `/synapse-router-artifact-creator` to modify an existing artifact | Wrong-tool detection → redirect to `/synapse-skill-skill-improver` |
| User invokes `/synapse-router-artifact-creator` without a specific idea yet | Wrong-tool detection → redirect to `/synapse-router-artifact-brainstormer` |
| Multiple artifacts in one session | Not supported — concurrency contract: ONE artifact per invocation; multi-artifact sessions use parallel synapse-router-artifact-creator agents |
...
| `/synapse-router-artifact-gatekeeper` | produces for | Every artifact synapse-router-artifact-creator produces must pass gatekeeper before promotion from `status: draft` |
...
6. `status-draft-mark($TYPE, $meta)` — ensure `status: draft` in registry row until `/synapse-router-artifact-gatekeeper` approves
...
[NEW] (wrong-tool) → [U] (understand goal) → [B] (baseline test, "is this needed?")
    → [W] (write skill, calls shared-steps) → [V] (validate via /synapse-skill-skill-improver) → [END]
```

Drop skill-creator's self-improvement infrastructure: SCOPE.md, PROGRAM.md, research/ dir, test-inputs/ dir, "Execution scope: ignore" lines — those belong at synapse-router-artifact-creator root if wanted, not per-flow. Replace inline frontmatter/registry/README writes with `shared-steps:*` calls. Load `design-principles-skill.md` at `[W]`, `templates/skill/` at scaffold step.
...
Source: clean-room. Grounded in `synapse-git-dispatch-cr` precedent (TOOL.md + script + test/ shape). Target ~80–100 LOC.
...
EXCLUDES grading the produced artifact's body quality — that is handled downstream by `write-{type}-eval` + `/synapse-router-artifact-gatekeeper`.
...
Side effect: pre-commit lifecycle hook auto-demoted 10 previously-stable skills to `draft` in `registry/SKILL_REGISTRY.md` (skill-router, autonomous-orchestrator, parallel-agents-dispatch, stakeholder-reviewer, protocol-creator, synapse-skill-skill-improver, skill-creator, synapse-router-artifact-brainstormer, synapse-router-artifact-gatekeeper, write-skill-eval). Re-run `/synapse-router-artifact-gatekeeper` on these after synapse-router-artifact-creator lands.

### CC2 — Sunset migration (pending)

Actionable only after synapse-router-artifact-creator is built, all 4 type-flows pass EVAL, and cross-check passes (run synapse-router-artifact-creator on a known-good prior creation, compare output to original creator output).

Cross-check coexistence concern: during the cross-check window both old creators and synapse-router-artifact-creator coexist. Confirm pre-commit hook tolerates temporary duplicate registry rows and that name-uniqueness check does not fire on type-renamed skills.
...
- `synapse-router-artifact-brainstormer` wrong-tool refs: `/skill-creator` → `/synapse-router-artifact-creator skill`
- `synapse-skill-skill-improver` references to old creator names
...
Before deleting old creators, produce an explicit inventory checklist comparing pre-deletion state vs post-migration state in synapse-router-artifact-creator references. Nothing may be lost.
...
- **`agents/` subdirectory inside synapse-router-artifact-creator:** per-flow `agents/{skill,protocol,agent,tool}/` subdirectories confirmed (turn-6 decision B). Creator confirms exact symlink targets at build time.
- **EVAL.md for synapse-router-artifact-creator itself:** routing correctness + per-type mechanical correctness (4 fixed prompts) confirmed scope. Creator decides exact metric thresholds.
...
- **Tool's frontmatter handling:** TOOL.md has YAML frontmatter (per synapse-git-dispatch-cr); script file itself does not. Creator confirms convention.
...
- **Follow-on consolidations (NOT in scope):** `synapse-improver` (consolidate synapse-skill-skill-improver + analogues), `synapse-router-eval-writer` (consolidate write-{skill,protocol,agent,tool}-eval). Same router pattern. Separate brainstorms after synapse-router-artifact-creator pattern lands.
