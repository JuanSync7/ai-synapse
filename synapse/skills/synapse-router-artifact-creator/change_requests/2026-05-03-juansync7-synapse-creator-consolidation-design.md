# Design Document — synapse-router-artifact-creator Consolidation

> Brainstorm slug: `2026-05-03-synapse-router-artifact-creator-consolidation`
> Status: **complete** | Artifact: skill (creation) | Target: `synapse/skills/synapse-router-artifact-creator/`
...
What changes: a single `synapse-router-artifact-creator` skill replaces all existing creators, using router-based progressive disclosure so each session loads only the flow relevant to the requested type.
...
`design-principles-skill.md` is moved from `skill-creator/references/` into `synapse-router-artifact-creator/references/`, not copied. Same for `design-principles-protocol.md` from `protocol-creator/references/`. During the cross-check window both old creators and synapse-router-artifact-creator coexist, but the moved files are the single source. No duplication.
...
synapse-router-artifact-creator's own EVAL covers: structural conformance, routing correctness (correct flow loaded for each type), and mechanical correctness (four fixed prompts assert artifact directory created, frontmatter valid, registry row added, README row added, EVAL/test scaffold generated). It does NOT grade the body quality of produced artifacts — that is the downstream job of `write-{type}-eval` + `synapse-router-artifact-gatekeeper`.

**Implication:** synapse-router-artifact-creator is accountable for the scaffold, not the content. This keeps synapse-router-artifact-creator's EVAL deterministic and automation-friendly, while pushing subjective quality judgment to the tools that already handle it.
...
**(CC1) Taxonomy migration — DONE.** Committed at `166086a` on `develop`. 24 framework artifacts migrated to `domain: synapse` (16 skills, 5 agents, 3 protocols). Pre-commit lifecycle hook auto-demoted 10 previously-stable skills to `draft` in `registry/SKILL_REGISTRY.md` — these require `/synapse-router-artifact-gatekeeper` re-runs downstream of this brainstorm.

**(CC2) Sunset migration — pending.** Blocked on: synapse-router-artifact-creator build + EVAL pass + cross-check against a known-good prior creation. Cross-check is the go/no-go gate. After cross-check passes:
- Delete `skill-creator/`, `protocol-creator/`, `agent-creator/`
- Update `synapse-router-artifact-brainstormer` wrong-tool refs: `/skill-creator` → `/synapse-router-artifact-creator skill`
- Update `synapse-skill-skill-improver` references to old creators
- Update `GOVERNANCE.md` if it references old creators
- Update `registry/SKILL_REGISTRY.md` rows

During the cross-check window, old creators and synapse-router-artifact-creator coexist. Pre-commit hook name-uniqueness check may fire — confirm tolerance before committing cross-check window state.

**(CC3) Knowledge transfer audit — pending.** Before deleting old creators, produce an explicit inventory checklist: sub-flows, design principles, hard-gates, HITL checkpoints, wrong-tool rules, templates, companion agents, per-node references, and EVAL criteria from each existing creator — compared against synapse-router-artifact-creator references to confirm nothing is lost.

**Design-principles MOVE (not copy).** `skill-creator/references/skill-design-principles.md` is moved (not copied) to `synapse-router-artifact-creator/references/design-principles-skill.md`. Same for protocol. During the cross-check window, old creators lose their design-principles refs — this is deliberate fragility that accelerates the transition.

---

## 8. Concurrency and Invocation Contract

synapse-router-artifact-creator handles exactly ONE artifact per invocation. Multi-artifact sessions use parallel synapse-router-artifact-creator agents, one per artifact. A single invocation MUST NOT create multiple artifacts — this is the concurrency contract enforced by the `[ROUTE]` hard-gate.

Input: `/synapse-router-artifact-creator <type> [name]`
...
Two sibling consolidations follow the same router pattern and become straightforward once synapse-router-artifact-creator lands:

- **synapse-improver** — consolidates `synapse-skill-skill-improver` + future protocol/agent/tool improvers
- **synapse-router-eval-writer** — consolidates `write-skill-eval`, `write-protocol-eval`, `write-agent-eval`, and a future `write-tool-eval`
...
| `registry/SKILL_REGISTRY.md` | produces for | synapse-router-artifact-creator adds its own row at creation; shared-steps writes rows for artifacts it creates |
| `synapse-router-artifact-brainstormer` | produces for (wrong-tool) | synapse-router-artifact-brainstormer wrong-tool block must redirect to `/synapse-router-artifact-creator <type>` — updated at CC2 sunset |
| `synapse-skill-skill-improver` | produces for (wrong-tool) | synapse-router-artifact-creator wrong-tool block redirects existing-artifact modification to `/synapse-skill-skill-improver` |
| `synapse-router-artifact-gatekeeper` | produces for | created artifacts land at `status: draft`; gatekeeper is the certification path |
